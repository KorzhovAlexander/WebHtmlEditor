...
<script>
    ...
    export default {
        name: 'summernote-temp',
        components: {
            DxButton,
        },
        data() {
            return {
                editorData: '<table border="1">\n' +
        ...
        created() {
            this.elems = document.getElementById('#summernote').summernote('code');
        },
        methods:{
            onClick: function () {
                console.log(this.editorData);
                window.open(`api/documents?htmlText=${this.editorData}`)
            },
            emptyEditor() {
                this.editorData = '';
            }
